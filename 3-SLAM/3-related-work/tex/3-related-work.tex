...
\midrule
Low Light & Feature extraction failure \cite{schmidtROVERMultiSeasonDataset2024} & Minimal (Active sensing) & Requires adaptive thresholds \cite{schmidtROVERMultiSeasonDataset2024} \\ 
\addlinespace
...
\addlinespace
Low Texture & Tracking loss \cite{linguaImprovingImageAlignment2025} & Geometric degeneracy \cite{soumyadipIntroductionLiDARSLAM2024} & High dependency on sensor fusion \cite{weiLiDARVisualInertial2026} \\ 
\addlinespace
...
Traditional optimisation focuses on minimising a scalar error metric, such as Absolute Trajectory Error (ATE), on a single ``representative'' dataset --- typically one of the prevailing SLAM benchmark datasets such as KITTI or TUM RGB-D. Yet this approach tends to ``chase'' the noise of that specific trajectory \cite{kandasamyTuningHyperparametersGrad}, resulting in parameters that are essentially ``hard-coded'' to that specific sequence of sensor observations and noise realisations of the training data. 
