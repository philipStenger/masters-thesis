...
to estimate dense disparity maps, and derives coloured 3D point clouds that are segmented to retain vine trunk structure. The LiDAR pipeline operates on dense point clouds accumulated from a Livox solid-state sensor over a commanded interval, using Patchwork++ for ground plane removal, and applying a multi-stage filtering sequence to isolate vine wood. Both pipelines share the same GICP registration module and the same pose correction mechanism; they differ only in their preprocessing steps, which are tailored to the noise characteristics and geometric coverage of the respective sensor.

The initial pose estimate consumed by the pipeline is provided by the ENU-aligned SLAM system described in \cref{sec:slam-methodology}. This pose locates the robot within the global coordinate frame and is used both to spatially crop the target cloud extracted from the 3DGS model and to generate perturbation hypotheses for GICP registration, as well as to anchor the final corrected transform in the global frame. The pipeline dataflow for each sensing modality is illustrated in
...
\begin{figure}[htbp]
  ...
  \caption{LiDAR preprocessing branch of the precise-alignment pipeline. Dense
           ...
           processing to produce a filtered source point cloud passed to the
...
a stereo foundation model that produces disparity maps with strong zero-shot generalisation across sensor configurations and scene types (see \cref{subsec:stereo-depth} for a description of the model and its benchmark performance). % TODO: clarify which FoundationStereo checkpoint is used and whether any fine-tuning was applied, or whether inference is purely zero-shot (OQ4)
...
This configuration serves two complementary purposes. First, the lateral offset provides marginally different viewing angles of the vine surface, yielding a denser and more representative composite point cloud than either camera alone. Second, the substantial overlap between the two clouds could in principle enable consistency-based outlier rejection, in which depth estimates present in one cloud but absent or inconsistent in the other are identified as likely erroneous stereo returns; this capability is not exploited in the current implementation, where the two clouds are concatenated without cross-validation, but represents a potential avenue for future improvement. This stands in contrast to a configuration in which the two cameras view the vine from opposite or widely separated directions, where each cloud would have to be accepted independently, with no means of cross-validation.
...
\begin{figure}[htbp]
  ...
  \caption{Dual-camera point cloud fusion. Both ZED cameras are mounted side by side
           ...
           between the clouds could in principle enable consistency-based outlier
           rejection; in the current implementation the clouds are concatenated
           directly before passing to trunk segmentation.}
...
The processing range is bounded to 0.1--3.0\,m to discard returns either too close to the sensor aperture or beyond the typical vine operating distance. The Ring-wise Normal-based Removal (RNR) and Ring-wise Vertical Point Filtering (RVPF) modules are enabled. Although these modules were originally designed for spinning LiDAR with regular scan rings, Patchwork++ assigns points to virtual concentric rings based on radial distance from the sensor origin regardless of the scan pattern; RNR and RVPF therefore operate on this spatial radial partition and remain applicable to the Livox non-repetitive pattern.
...
\textbf{(b) Target cloud construction.} A single target point cloud is constructed once from the Gaussian means of the 3DGS model, as described in \cref{subsec:pa-reference-model}. This cloud is fixed for all registration hypotheses within a given pose estimation step, eliminating the need to re-render the model at multiple viewpoints.

\textbf{(c) Neighbourhood perturbation.} To mitigate sensitivity to the accuracy of the initial SLAM pose, the pipeline generates 35 candidate initial poses by applying small rigid-body perturbations to the initial estimate $T_0$. Each perturbed pose $T_k$ is formed by right-multiplying $T_0$ with a perturbation matrix $\Delta T_k$:
%
\begin{equation}
    T_k = T_0 \cdot \Delta T_k, \qquad k = 0, \ldots, 34.
    \label{eq:perturbation}
\end{equation}
%
Right-multiplication ensures that perturbations are applied in the body frame of the robot, so that translations and rotations act along the robot's local axes rather than the global frame axes. The 35 hypotheses are organised into five groups according to the structure of $\Delta T_k$, as summarised in \cref{tab:pa-perturbation-groups}. The perturbation magnitudes $\delta_t$ (translation) and $\delta_r$ (rotation) are configurable parameters whose values are reported in \cref{tab:pa-icp-params}. The perturbation scheme and subsequent hypothesis selection are illustrated in \cref{fig:pa-icp-candidates}.
...
\begin{figure}[htbp]
  ...
  \caption{GICP perturbation hypothesis scheme. Thirty-five candidate poses are generated
           by applying structured rigid-body perturbations (Orig and Groups~A, B, C,~and~E; \cref{tab:pa-perturbation-groups})
           to the initial SLAM pose; each hypothesis is registered against the single
           target point cloud extracted from the 3DGS model. The hypothesis whose
           registration achieves the highest fitness score above the acceptance threshold
           (and lowest RMSE as a tiebreaker) is selected as the final alignment.
           }
...
\begin{figure}[htbp]
  ...
  \caption{Graduated GICP registration loop for a single perturbation hypothesis.
           ...
           that clamps the total accumulated pitch and roll correction to $\pm 5^{\circ}$.
...
\paragraph{Note on the stereo modality.}
The graduated loss strategy and rotation constraint described above are applied only when the source cloud originates from the LiDAR pipeline. Despite the multi-stage filtering described in \cref{subsubsec:pa-lidar-processing}, the LiDAR source cloud retains a higher residual outlier fraction than the stereo cloud because the geometric filtering criteria cannot distinguish vine wood from structurally similar non-vine objects (e.g.\ the mounting frame or nearby surfaces) that fall within the azimuthal and range filters. For the stereo modality, the trunk segmentation pipeline substantially reduces the outlier fraction prior to registration, and the standard GICP objective without a robust kernel is used.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{} >{\raggedright}p{0.27\textwidth} >{\raggedright}p{0.20\textwidth} >{\raggedright\arraybackslash}X @{}}
    ...
    Rotation constraint (LiDAR)     & $\pm 5^{\circ}$ pitch/roll & Clamp on total accumulated ICP correction; yaw unconstrained \\
...
\begin{figure}[htbp]
  ...
  \caption{Pipeline execution model. Each localisation request is dispatched as a
           ...
           which executes them in a dedicated worker thread pool. The GPU-resident
           FoundationStereo inference stage is protected by a semaphore concurrency
           guard; target cloud extraction from the 3DGS model is a CPU-bound read of
           Gaussian mean positions and does not require GPU access. Incoming requests
           are queued independently, ensuring each observation is processed in full.}
  \label{fig:pa-execution-model}
\end{figure}

The GPU-resident FoundationStereo inference stage is governed by a semaphore-based concurrency guard that prevents simultaneous kernel execution from exceeding available GPU memory. Target cloud extraction from the 3DGS model reads Gaussian mean positions on the CPU and is not subject to this guard. When a new localisation request arrives while a prior job is still processing, the new job is queued independently and executes after the in-progress job completes, ensuring that each sensor observation is processed in full without resource contention. This execution model supports both the stereo and LiDAR pipelines without modification, as the modality-specific processing difference is captured entirely within the command sequence submitted for each observation type.

The preceding subsections have described the complete localisation pipeline: sensor preprocessing for both stereo and LiDAR observations, target point cloud extraction from the 3DGS model, GICP registration with neighbourhood perturbation, and pose correction. The following subsection describes the experimental setup designed to evaluate the LiDAR pipeline under controlled conditions. The results of these experiments are presented in \cref{sec:precise-alignment-results}.
...
This section describes the experimental setup used to evaluate the LiDAR precise-alignment pipeline. The stereo experimental setup is addressed in a subsequent section. Six aspects of the evaluation are described: the test environment, the vine specimen and reference model, the trial design, the construction of initial poses, the evaluation protocol, and the metrics used to validate alignment quality.
...
Trials were conducted in an indoor laboratory environment under controlled conditions. This setting was chosen to isolate the performance of the localisation pipeline from environmental confounders present in outdoor vineyards, such as solar irradiance variation, wind-induced vine motion, and GNSS multipath interference, all of which would introduce additional sources of variability that cannot be independently controlled.

Outdoor validation was not feasible for this evaluation for the following reason. The 3DGS reference model was georeferenced during outdoor data collection at a specific site, encoding the vine's position within the ENU coordinate frame tied to that location. The robot was subsequently relocated to a different facility, creating a spatial mismatch in the global frame that prevents the outdoor model from being used directly for live registration trials without re-georeferencing. The indoor laboratory accordingly served as a controlled proxy environment; outdoor validation under realistic field conditions remains future work.

Several limitations of the indoor evaluation should be noted. First, the laboratory lighting and surface reflectance properties differ from outdoor vineyard conditions; LiDAR return intensity and noise characteristics may therefore not be representative of field operation. Second, only a single vine specimen was present, eliminating inter-vine geometric ambiguity --- a significant simplification relative to a vineyard row containing many structurally similar vines. Third, the 3DGS reference model was reconstructed outdoors and manually re-aligned to the laboratory coordinate system; any residual error in this manual alignment is indistinguishable from pipeline registration error in the reported metrics. The physical arrangement of the laboratory, including the vine specimen, robot platform, and sensor positions, is illustrated in \cref{fig:pa-lab-setup}.
...
Trials were organised as a structured evaluation across nine operating conditions defined by two factors: the distance from the sensor to the vine trunk ($d$) and the lateral approach angle ($\theta$) relative to the vine normal. Three levels were selected for each factor: $d \in \{40, 60, 80\}$\,cm and $\theta \in \{-15^{\circ}, 0^{\circ}, {+}15^{\circ}\}$, yielding nine unique conditions that sample the operational envelope at representative points. One additional replicate at the centre condition ($d = 60$\,cm, $\theta = 0^{\circ}$) was included as a repeatability check, resulting in ten trials in total. Because only a single observation is available per non-centre condition, the design is exploratory rather than statistically powered; results are interpreted as characterising pipeline behaviour across the range of expected operating conditions rather than estimating main effects or interactions.

Trials were executed in randomised order to mitigate systematic effects arising from sensor warm-up drift or any time-correlated environmental change. A settling period was observed between consecutive trials --- the operator waited until the robot was visibly stationary and any transient sensor artefacts from the preceding repositioning had subsided before initiating data acquisition.
...
\begin{table}[htbp]
  \centering
  \caption{LiDAR experimental trial matrix. Each row defines one trial with its distance and angle factor levels. Trial~D60A0r is a replicate of the centre condition (60\,cm, 0$^{\circ}$).}
...
\begin{figure}[htbp]
  ...
  \caption{Per-trial evaluation protocol. The initial pose is perturbed to
           ...
           result is selected. Three complementary validation strategies ---
           perturbation convergence consistency, inter-trial consistency, and
