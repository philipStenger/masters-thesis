...
This section presents the empirical results from the evaluation of three localisation configurations: the baseline GLIM algorithm with default parameters, the refined GLIM algorithm with optimised parameters, and GLIM-GNSS. Each configuration was evaluated on five parameterisation datasets; additionally, all three configurations were evaluated on a held-out Evaluation Dataset, described in \cref{subsec:slam-experimental-setup}, to assess generalisation to an unseen route. Performance is reported using Root Mean Square Error (RMSE), Absolute Trajectory Error (ATE), Relative Pose Error (RPE), and, for GLIM-GNSS, the Global Alignment and Smoothness Profile (GASP) metric defined in the methodology.
...
\subsection{GLIM-GNSS Performance}

\cref{tab:proposed_performance} reports the results for GLIM-GNSS. The error percentage per metre ranges from 0.0134\% (Dataset~3) to 0.0501\% (Dataset~2). The 3D RMSE ranges from 0.1031\,m (Dataset~3) to 0.2280\,m (Dataset~2), and the ATE ranges from 0.0711\,m (Dataset~3) to 0.1741\,m (Dataset~2). The GASP metric, which combines drift rate and local pose smoothness, returned values between 0.0274 (Dataset~0) and 0.0679 (Dataset~2).

\begin{table}[htbp]
\centering
\caption{GLIM-GNSS performance over the 5 test datasets}
\label{tab:proposed_performance}
\begin{tabularx}{\textwidth}{lXXXXX}
\toprule
\textbf{Metrics} & \textbf{Dataset 0} & \textbf{Dataset 1} & \textbf{Dataset 2} & \textbf{Dataset 3} & \textbf{Dataset 4} \\ \midrule
RMSE\ x\ (m)\ & 0.0756 & 0.0528 & 0.0909 & 0.0453 & 0.0728 \\
RMSE\ y\ (m)\ & 0.0645 & 0.0906 & 0.2003 & 0.0880 & 0.0789 \\
RMSE\ z\ (m)\ & 0.0465 & 0.0347 & 0.0599 & 0.0290 & 0.0560 \\
RMSE\ 3D\ (m)\ & 0.1097 & 0.1105 & 0.2280 & 0.1031 & 0.1211 \\
ATE\ (m)\ & 0.0846 & 0.0811 & 0.1741 & 0.0711 & 0.1031 \\
max\ error\ 3D\ (m)\ & 0.7363 & 0.8963 & 1.0246 & 0.9132 & 0.7056 \\
error\ per\ meter\ (m)\ & 0.0002 & 0.0002 & 0.0005 & 0.0001 & 0.0002 \\
error\ percent\ per\ meter\ (\%)\ & 0.0171 & 0.0154 & 0.0501 & 0.0134 & 0.0153 \\
gasp & 0.0274 & 0.0290 & 0.0679 & 0.0295 & 0.0429 \\
RPE\ 1m\ translation\ RMSE\ & 0.0270 & 0.0308 & 0.0556 & 0.0315 & 0.0444 \\
RPE\ 1m\ translation\ mean\ & 0.0175 & 0.0206 & 0.0282 & 0.0202 & 0.0249 \\
RPE\ 1m\ translation\ std\ & 0.0205 & 0.0229 & 0.0479 & 0.0240 & 0.0368 \\
RPE\ 1m\ translation\ max\ & 0.2001 & 0.4906 & 0.6992 & 0.5074 & 0.6462 \\
RPE\ 10m\ translation\ RMSE\ & 0.0649 & 0.0936 & 0.1772 & 0.1011 & 0.0769 \\
RPE\ 10m\ translation\ mean\ & 0.0433 & 0.0613 & 0.0987 & 0.0629 & 0.0573 \\
RPE\ 10m\ translation\ std\ & 0.0482 & 0.0708 & 0.1471 & 0.0709 & 0.0512 \\
RPE\ 10m\ translation\ max\ & 0.3799 & 0.7620 & 0.8566 & 0.8628 & 0.5377 \\ \bottomrule
\end{tabularx}
\end{table}
...
\multirow{3}{=}{error percent per meter (\%)} & Baseline GLIM & 3.3580 & 1.2530 & 7.3610 & 4.8970 & 4.1910 \\
 ...
 & GLIM-GNSS & \textbf{0.0171} & \textbf{0.0154} & \textbf{0.0501} & \textbf{0.0134} & \textbf{0.0153} \\ \midrule
...
\multirow{3}{=}{RPE 1m translation std} & Baseline GLIM & 0.4125 & 0.2602 & 0.6767 & 0.9384 & 0.4285 \\
 ...
 & GLIM-GNSS & \textbf{0.0205} & \textbf{0.0229} & \textbf{0.0479} & \textbf{0.0240} & \textbf{0.0368} \\ \midrule
...
\multirow{3}{=}{error per meter (m)} & Baseline GLIM & 0.0340 & 0.0130 & 0.0740 & 0.0490 & 0.0420 \\
 ...
 & GLIM-GNSS & \textbf{0.0002} & \textbf{0.0002} & \textbf{0.0005} & \textbf{0.0001} & \textbf{0.0002} \\ \bottomrule
\end{tabularx}
\end{table}

\subsection{GNSS Factor Utilisation}

\cref{tab:gnss_stats} reports the number of GNSS factors incorporated into the GLIM-GNSS system's factor graph relative to the total number of available GNSS measurements. Across the five datasets, the proportion of GNSS points retained after outlier rejection and incorporated as factors ranges from 0.65\% (Dataset~2) to 0.78\% (Dataset~3).

\begin{table}[htbp]
\centering
\caption{Statistics on the number of GNSS factors used for GLIM-GNSS}
...
\cref{fig:comparison_progression} plots the best consensus error at the end of each optimisation stage. \cref{fig:baseline_progression} tracks the error percentage for the baseline GLIM, and \cref{fig:proposed_progression} tracks the GASP metric for GLIM-GNSS. The baseline error decreases monotonically across the four stages; the GLIM-GNSS system's GASP score increases from stage~0 to stage~3.
...
\cref{fig:comparison_spider} presents four radar plots comparing the baseline GLIM, refined GLIM, and GLIM-GNSS across all five datasets. The four metrics displayed are error per metre, error percentage per metre, ATE, and RPE 1\,m translation standard deviation. Radial axes are plotted on a logarithmic scale.
...
\begin{figure}[htbp]
     ...
     \end{subfigure}
     \caption{Spider plot comparisons of the baseline GLIM, refined GLIM, and GLIM-GNSS configurations for key performance metrics.}
...
\subsubsection{GLIM-GNSS --- Evaluation Dataset}

\cref{tab:eval_proposed_performance} reports the GLIM-GNSS system's performance on the Evaluation Dataset. The mean error percentage per metre is 0.0166\% with a standard deviation of 0.0004\%, indicating highly consistent performance across all five runs. The mean RMSE 3D is 0.1459\,m and the mean ATE is 0.1135\,m. The GASP metric has a mean of 0.0508 (std 0.0081), which is consistent with the range observed on the parameterisation datasets (0.0274--0.0679).

\begin{table}[htbp]
\centering
\caption{GLIM-GNSS performance on the Evaluation Dataset (mean, std, median, min, and max across five independent runs)}
...
\cref{tab:eval_comparison_key_metrics} presents a side-by-side comparison of the three methods on three key metrics across the Evaluation Dataset. GLIM-GNSS achieves the lowest mean value for all three metrics. The refined GLIM's mean RPE 1\,m translation standard deviation exceeds that of the baseline due to the contribution of the catastrophic failure run; the median values for this metric restore the expected baseline--refined--GLIM-GNSS ordering.
...
\multirow{3}{=}{error percent per meter (\%)}
 ...
 & GLIM-GNSS & \textbf{0.0166} & 0.0004 & 0.0165 \\ \midrule

\multirow{3}{=}{RPE 1m translation std}
 & Baseline GLIM  & 0.1093 & 0.0657 & 0.0952 \\
 & Refined GLIM   & 0.4019 & 0.8213 & 0.0367 \\
 & GLIM-GNSS & \textbf{0.0441} & 0.0080 & 0.0454 \\ \midrule

\multirow{3}{=}{error per meter (m)}
 & Baseline GLIM  & 0.0196   & 0.0045    & 0.0198   \\
 & Refined GLIM   & 0.0062   & 0.0011    & 0.0065   \\
 & GLIM-GNSS & \textbf{0.000166} & 0.0000035 & 0.000165 \\ \bottomrule

\end{tabularx}
\end{table}

\paragraph{Statistical Separation of Methods}
With only five independent runs per configuration, formal hypothesis testing has limited statistical power; a two-tailed Wilcoxon signed-rank test with $n = 5$ pairs cannot achieve a $p$-value below 0.0625. We therefore characterise the separation between methods using three complementary arguments.

First, the distributions are entirely non-overlapping on the primary metric (error percentage per metre): the GLIM-GNSS system's worst run (0.0172\%) is 26$\times$ lower than the refined GLIM's best run (0.4453\%), which is itself 3$\times$ lower than the baseline's best run (1.339\%). No permutation of run labels can bring any two adjacent methods' distributions into overlap.

Second, applying a $t$-based 95\% confidence interval to the GLIM-GNSS system's error percentage per metre yields $0.0166 \pm t_{0.025,4} \cdot 0.0004/\sqrt{5} = [0.0161\%,\; 0.0171\%]$. The corresponding interval for the refined GLIM is $[0.4864\%,\; 0.7566\%]$ and for the baseline $[1.4003\%,\; 2.5221\%]$. These intervals do not overlap, confirming that the observed differences exceed what could be attributed to run-to-run variability.

Third, the effect size between GLIM-GNSS and the refined GLIM, measured by Cohen's $d$ using the pooled standard deviation, exceeds $d = 7.8$ --- well beyond the threshold conventionally considered ``large'' ($d = 0.8$). The practical magnitude of the improvement (approximately one order of magnitude relative to the refined GLIM, and two orders relative to the untuned baseline) is therefore unambiguous regardless of the limited sample size.